...
const BAZAARLINK_MOD  = (process.env.BAZAARLINK_MODEL || 'auto:free').trim();
const OPENROUTER_MOD  = (process.env.OPENROUTER_MODEL || 'google/gemini-2.5-flash:free').trim();
...
async function callOpenRouter(systemPrompt, userPrompt, temperature = 0.85, maxTokens = 2000) {
  let lastError;
  let modelToUse = OPENROUTER_MOD;
  if (modelToUse !== 'openrouter/free' && !modelToUse.endsWith(':free')) {
...
async function callBazaarLink(systemPrompt, userPrompt, temperature = 0.6, maxTokens = 2000) {
  let lastError;
  let modelToUse = BAZAARLINK_MOD;
  if (modelToUse !== 'auto:free' && !modelToUse.endsWith(':free')) {
    modelToUse = modelToUse + ':free';
  }
...
app.post('/api/article', authMiddleware, async (req, res) => {
  ...
  const isFreeModel = !OPENROUTER_KEY || OPENROUTER_MOD.includes(':free') || OPENROUTER_MOD === 'openrouter/free';
  if (isFreeModel && !BAZAARLINK_KEY) {
    return res.status(500).json({ error: 'BazaarLink API key is not configured on the server. Please add it to your project environment variables for free generation fallback.' });
  }
  if (!isFreeModel && !OPENROUTER_KEY) {
...
{"title":"A compelling title (max 10 words)","content":"Full article. Paragraphs separated by \\n\\n","keyTakeaways":["Takeaway 1","Takeaway 2","Takeaway 3"],"difficulty":"C1"}`;
  ...
  try {
    let data;
    if (isFreeModel) {
      console.log(`[Routing /api/article to BazaarLink auto:free for free generation]`);
      data = await callBazaarLink(systemPrompt, userPrompt, 0.85, 1800);
    } else {
      console.log(`[Calling OpenRouter model ${OPENROUTER_MOD} for generation]`);
      data = await callOpenRouter(systemPrompt, userPrompt, 0.85, 1800);
    }
...
app.post('/api/translate/feedback', authMiddleware, async (req, res) => {
  const { originalText, userTranslation } = req.body;
  if (!originalText || !userTranslation) {
    return res.status(400).json({ error: 'Missing originalText or userTranslation' });
  }
  ...
  const systemPrompt = `You are a professional literary editor and languages tutor.
Compare the user's manual translation against the original text.
1. Dynamically identify the source language of the original text and the target language of the user's translation.
2. Highlight exactly what the user got right (1-3 aspects, like accurate phrasing, good vocabulary, or grammar).
3. Identify constructive improvements (awkward vocabulary, wrong syntax, grammar errors) with corrections and brief explanations.
4. Return your analysis ONLY as a valid JSON object matching this schema:
{
  "correct_aspects": ["Highlight 1", "Highlight 2"],
  "improvements": [
    {
      "original_phrase": "original word or phrase",
      "user_phrase": "user translated word or phrase",
      "correction": "suggested correction",
      "explanation": "brief description of why this correction is better"
    }
  ]
}
Return ONLY valid JSON. Do not include markdown code block wrappers or any extra text.`;
  ...
  try {
    let data;
    if (isFreeModel) {
      console.log(`[Routing /api/translate/feedback to BazaarLink auto:free for free generation]`);
      data = await callBazaarLink(systemPrompt, userPrompt, 0.3, 1200);
    } else {
      console.log(`[Calling OpenRouter model ${OPENROUTER_MOD} for generation]`);
      data = await callOpenRouter(systemPrompt, userPrompt, 0.3, 1200);
    }
    res.json(data);
  } catch (e) {
    console.error('[/api/translate/feedback]', e.message);
    res.status(500).json({ error: e.message });
  }
});
...
// Triggering watch restart to reload .env configuration v5
